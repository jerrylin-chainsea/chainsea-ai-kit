  ...
  <div style={{ display: 'flex', height: 540, background: '#1e1e1e', borderRadius: 14, overflow: 'hidden', border: '1px solid #2b2b2b', boxShadow: '0 18px 50px -28px rgba(0,0,0,0.55)' }}>
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div style={{ flex: 1, background: '#1e1e1e', padding: '16px 22px', fontFamily: mono, fontSize: 17, color: '#9aa0a6' }}>
        ...
        <div style={{ paddingLeft: 24 }}><span style={{ color: '#9cdcfe' }}>name</span>: <span style={{ color: '#ce9178' }}>'NOVA WAREHOUSE'</span>,</div>
        <div>{'}'}</div>
      </div>
...
const Outcome: Page = () => (
  <RealShot img={weblabShot} url="localhost:5180" eyebrow="今天的共同練習場" title="NOVA WAREHOUSE:四個工具都圍繞它示範" caption="VSCode 段打開它、Git 段改首頁標題並存檔、Claude Code / Codex 段用它示範 AI 工具如何被控制。網頁上方可以切到倉儲後台、訂單可視化與 LINE 推播中心。" foot="U11 · 課1 · 成品預覽" maxH={530} pos="center" />
);
const Why: Page = () => (
...
const Sec1: Page = () => <Section no="1" title="VSCode:你的專案工作台" time="0:15 - 1:20" sub="先開 RAR 解壓後的教材根目錄,再用 cd 走進 web-lab,用 npm 把 NOVA WAREHOUSE 跑起來。" />;
...
const WebReal: Page = () => <RealShot img={weblabShot} url="localhost:5180" eyebrow="段 1 · 成功畫面" title="看到 NOVA WAREHOUSE,這段就成功" caption="這不是截圖假裝,而是本機 web-lab 跑起來後的畫面。之後 Git、Claude Code、Codex 都以它當練習場。" foot="U11 · 課1 · VSCode" maxH={530} pos="center" />;
const VsPit: Page = () => <div style={fill}><div style={pad}><Eyebrow>段 1 · 卡住了?</Eyebrow><Title size={50}>VSCode / npm 常見狀況</Title><div className="ts-rise" style={{ marginTop: 28, maxWidth: 1500 }}><Pitfall items={[['找不到終端機:', '上方選單 Terminal → New Terminal。'], ['npm 說找不到 package.json:', '你還沒 cd 到 web-lab。'], ['npm run dev 停住:', '正常,代表網站正在跑,不要關。']]} /></div><div style={{ marginTop: 18 }}><Checkpoint items={['VSCode 開啟教材根目錄', '會開 Terminal', '成功執行 npm install', '成功執行 npm run dev', 'localhost 看得到 NOVA WAREHOUSE']} /></div></div><Foot label="U11 · 課1 · VSCode" /></div>;
...
const EditDo: Page = () => <div style={fill}><div style={pad}><Eyebrow>段 2 · 先製造一個改動</Eyebrow><Title size={50}>改 data.js 裡的首頁標題</Title><div className="ts-rise" style={{ display: 'flex', flexDirection: 'column', gap: 18, marginTop: 30 }}><Atom n={1} act={<span>打開 <Key>web-lab/src/data.js</Key>。</span>} see="看到 name: 'NOVA WAREHOUSE'。" /><Atom n={2} act="把引號裡的標題加上自己的名字。" see="瀏覽器畫面跟著變。" /><Atom n={3} act="按 Ctrl+S 存檔。" see="VSCode 分頁上的小點消失。" /><Harvest>要先有改動,Git 才有東西可以看。</Harvest></div></div><Foot label="U11 · 課1 · Git" /></div>;
