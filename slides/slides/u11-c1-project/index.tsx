...
const EditDo: Page = () => <div style={fill}><div style={pad}><Eyebrow>段 2 · 先製造一個改動</Eyebrow><Title size={50}>改 data.js 裡的首頁標題</Title><div className="ts-rise" style={{ display: 'flex', flexDirection: 'column', gap: 18, marginTop: 30 }}><Atom n={1} act={<span>打開 <Key>web-lab/src/data.js</Key>。</span>} see="看到 name: 'BOBA TIDE'。" /><Atom n={2} act="把引號裡的標題加上自己的名字。" see="瀏覽器畫面跟著變。" /><Atom n={3} act="按 Ctrl+S 存檔。" see="VSCode 分頁上的小點消失。" /><Harvest>要先有改動,Git 才有東西可以看。</Harvest></div></div><Foot label="U11 · 課1 · Git" /></div>;
