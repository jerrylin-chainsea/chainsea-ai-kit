...
import ghPagesSourceShot from './assets/gh-pages-source.png';
import ghActionsDeployShot from './assets/gh-actions-deploy-success.png';
...
const McpChrome: Page = () => (
  <div style={fill}><div style={pad}><Eyebrow>段 2 · MCP 一 · 七要素</Eyebrow><Title size={44}>Chrome DevTools MCP:讓 AI 自己驗收 web-lab</Title>
    <div className="ts-rise" style={{ marginTop: 22 }}><SevenCard title="chrome-devtools" tag="開源 · 需 Node + Chrome" rows={[
      ['用途', '讓 AI 開真的 Chrome:查 console error、看 Network payload、檢查畫面元素 ── 截圖只是附帶證據。'],
      ['安裝', <><Key>claude mcp add chrome-devtools -- npx chrome-devtools-mcp@latest</Key></>],
      ['固定操作', <>先 <Key>npm run dev</Key> 跑起 web-lab → 請 AI 開 localhost 逐項檢查畫面與行為。</>],
      ['固定 prompt', '「打開 http://localhost:5180。檢查:1.console有沒有error 2.品牌入口是否有動態背景 3.訂單看板是否有canvas動畫 4.LINE推播中心是否看得到三個推播範本。請回報結果與一張截圖。」'],
      ['預期結果', 'AI 回報 4 項檢查結果,不是只交一張圖。'],
      ['驗收', <><Key>/mcp</Key> 列出 chrome-devtools;4 項回報對得起來,截圖只是附件。</>],
      ['保底', '連不上 → 自己在瀏覽器按 F12 手動檢查,不擋進度。'],
    ]} /></div>
  </div><Foot label="U11 · 課4 · 段2" /></div>
);
const McpContext7: Page = () => (
  <div style={fill}><div style={pad}><Eyebrow>段 2 · MCP 二 · 七要素</Eyebrow><Title size={44}>Context7 MCP:查套件最新文件</Title>
    <div className="ts-rise" style={{ marginTop: 22 }}><SevenCard title="context7" tag="開源 · 選填 API 金鑰" rows={[
      ...
      ['固定 prompt', '「Astro 部署到 GitHub Pages 的最新設定方式是什麼?use context7」'],
      ['預期結果', 'AI 依最新文件回答,而不是憑記憶 —— 順便替等一下的 blog 部署鋪路。'],
...
const McpCodebaseMemory: Page = () => (
  <div style={fill}><div style={pad}><Eyebrow>段 2 · MCP 三 · 七要素</Eyebrow><Title size={44}>Codebase Memory MCP:不只查架構,還能查影響範圍</Title>
    <div className="ts-rise" style={{ marginTop: 22 }}><SevenCard title="codebase-memory-mcp" tag="開源 · 全本地 · 免 API key" rows={[
      ['用途', '把程式碼庫索引成知識圖譜:查架構總覽、呼叫關係,還能問「改這裡會影響哪裡」。'],
      ...
      ['固定操作', <>先請 AI <Key>index_repository</Key> 索引專案 → 再問架構或影響範圍。</>],
      ['固定 prompt', '「請先索引這個專案,回答:1.整體架構長怎樣?ShopConsole.jsx與shopLogic.js怎麼互動? 2.如果C2改shopLogic.js,會影響哪些畫面?」'],
      ['預期結果', 'AI 用圖譜回答架構,也能指出具體受影響的檔案與畫面,不是憑印象猜。'],
      ['驗收', <><Key>/mcp</Key> 列出 codebase-memory;兩類問題都答得出具體檔案關係。</>],
...
const NineOutcomes: Page = () => {
  const outs = ['一鍵啟動跑起專案', 'BOBA TIDE 首頁', '備料控制台 action rule', '訂單看板動態畫面', '一次 planner A–F 計畫與人審', '一次小範圍 shopLogic 修改 + git diff', '一份 LINE Flex payload 預覽', '一次 mock 送出 + [blocked] 驗證', '一次 ReAct 修錯(擋牌出現又消失)', '自建 /ops-check 與 /ship-check', '三個 MCP 親手裝起來', 'Astro 技術 blog + GitHub Pages 上線'];
  return (
...
const CoverMcp: Page = () => (
  <div style={fill}><div style={{ ...pad }}><Eyebrow>U11 · 第 4 堂 / 共 4 堂 · 4 小時 · 零基礎</Eyebrow>
    <div className="ts-rise" style={{ marginTop: 34 }}><div style={{ fontSize: 100, fontWeight: 900, letterSpacing: '-0.03em', lineHeight: 1.04 }}>幫 AI 接上<br />三條外部工具的線</div>
      <Lead>U3 你已經做過一次 AI 安全閘門(human-in-the-loop + token 邊界)。今天先把常提醒 AI 的規矩寫成 <B>Skills</B>,再進主線:<B>三個 MCP</B> ── 讓 AI 自己開瀏覽器驗收、查最新文件、記住整個專案。最後把成果寫成技術 blog,部署上線。</Lead></div>
  </div><div style={{ position: 'absolute', right: 110, bottom: 84, fontFamily: mono, fontSize: 21, color: C.faint }}>Skills · 三個 MCP · blog 部署</div><Foot label="U11 · 課4" /></div>
);
const WhyMcp: Page = () => (
...
const RoadmapMcp: Page = () => {
  const items = [['段 1', 'Skills', '把常提醒 AI 的規矩,寫成固定指令', '0:15'], ['段 2', '三個 MCP(主線)', '看畫面 / 查文件 / 記專案,親手裝', '0:45'], ['段 3', '安全 + blog', '權限三問、技術紀錄、部署上線', '2:45']];
  return (
...
const PainChrome: Page = () => (
  <Pain tag="MCP 一 · chrome-devtools" title="AI 說「改好了」,你怎麼知道畫面、資料都對?"
    pain={<>AI 回你「已完成」,但你得<B>自己開瀏覽器、自己按 F12</B> 看 console 有沒有錯、Network 傳的資料對不對,改一次查一次,很累。</>}
    fix={<>讓 AI <B>自己開 Chrome,查 console、看 Network payload、檢查畫面元素</B>,一次回報四件事,截圖只是附帶證據。</>} />
);
const McpChromeArtifact: Page = () => (
  <RealShot img={weblabHome} url="localhost:5180 · chrome-devtools MCP 檢查過" eyebrow="段 2 · chrome-devtools 產出" title="這不只是一張截圖,是 AI 自己驗收過的結果"
    caption="接上 chrome-devtools MCP 後,對 AI 說『開 localhost:5180,查 console error、動態背景、canvas 動畫、兩個推播範本』── 它會逐項回報,這張截圖只是附帶證據。驗收 web-lab 不用你自己一項一項查。" foot="U11 · 課4 · 段2" maxH={520} />
);
...
const PainCodebase: Page = () => (
  <Pain tag="MCP 三 · codebase-memory" title="專案一大,AI 不只會猜錯,還可能改壞不該動的地方"
    pain={<>問「ShopConsole 和 shopLogic 怎麼互動?」它得一檔一檔翻、常常猜錯;改東西前也不知道<B>會誤傷哪些畫面</B>。</>}
    fix={<>先 <Key>index_repository</Key> 把專案<B>索引成地圖</B>,架構問題秒答,也能先問「改這裡會影響哪裡」。</>} />
);
const McpDone: Page = () => (
...
const RecapMcp: Page = () => {
  const pts = [['Skills', '把常提醒 AI 的規矩,寫成 /ops-check、/ship-check 這類固定指令。'], ['chrome-devtools', '讓 AI 自己開瀏覽器,查 console、Network payload,驗收 web-lab。'], ['context7', '把最新官方文件餵給 AI,不再用過時 API。'], ['codebase-memory', '把專案索引成地圖,問架構也問改動影響範圍。'], ['blog + 部署', '寫成技術 blog,部署到 GitHub Pages 公開網址。']];
  return (
...
const BlogDeploy: Page = () => (
  <div style={fill}><div style={{ ...pad, display: 'grid', gridTemplateColumns: '1fr 720px', gap: 46, alignItems: 'center' }}>
    <div className="ts-rise" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div><Eyebrow>結業前 · 動手 · 部署上線</Eyebrow><Title size={44}>寫完不算完成,部署上線才算</Title></div>
      <Atom n={1} act={<>把 repo push 上 GitHub,打開 repo 的 <Key>Settings → Pages</Key>。</>} see="Source 選項出現。" />
      <Atom n={2} act={<>Source 選 <Key>GitHub Actions</Key>。</>} see="Pages 設定完成。" />
      <Atom n={3} act={<>到 <Key>Actions</Key> 分頁,執行 <Key>Deploy Astro Blog to GitHub Pages</Key>。</>} see="workflow 跑完顯示綠勾。" />
      <Atom n={4} act={<>打開 <Key>https://你的帳號.github.io/repo名稱/</Key>。</>} see="部落格公開網址打得開,文章與樣式正常。" />
      <Harvest>base path 不用手改:astro.config.mjs 會自動讀 GitHub Actions 給的 repo 名稱算出來。</Harvest>
    </div>
    <div style={{ background: '#fff', border: `1px solid ${C.line}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 18px 50px -28px rgba(30,30,40,0.4)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 18px', background: '#f0f2f5', borderBottom: `1px solid ${C.line}` }}><div style={{ display: 'flex', gap: 7 }}>{['#ff5f57', '#febc2e', '#28c840'].map((c, i) => <span key={i} style={{ width: 11, height: 11, borderRadius: '50%', background: c }} />)}</div><div style={{ flex: 1, background: '#fff', border: `1px solid ${C.line}`, borderRadius: 7, padding: '5px 14px', fontFamily: mono, fontSize: 15, color: C.faint }}>github.com/.../settings/pages</div></div>
      <img src={ghPagesSourceShot} alt="GitHub Pages Source: GitHub Actions" style={{ display: 'block', width: '100%', objectFit: 'cover' }} />
    </div>
  </div><Foot label="U11 · 課4 · Astro blog" /></div>
);
const BlogDeployArtifact: Page = () => (
  <RealShot img={ghActionsDeployShot} url="github.com/.../actions/runs/... · 真的跑過一次" eyebrow="結業前 · 部署產出" title="build 和 deploy 都成功,網址就在下面"
    caption="這是這個課程 repo 真的執行過一次 Deploy Astro Blog to GitHub Pages 的結果:build、deploy 兩個 job 都是綠勾,右下角就是算出來的公開網址。不是示意圖。" foot="U11 · 課4 · Astro blog" maxH={520} />
);

export const meta: SlideMeta = { title: 'U11 課4:Skills + MCP + Astro 技術紀錄與部署', createdAt: '2026-07-04T00:00:00.000Z' };

export const notes: (string | undefined)[] = [
  '各位同學好,最後一堂。今天分三塊:先把常提醒 AI 的規矩寫成 Skills,接著主線 —— 三個 MCP,讓 AI 自己開瀏覽器驗收、查最新文件、記住整個專案,最後把成果寫成技術 blog 並部署上線。',
  ...
  '看路線。三段:段一 Skills(把規矩寫成固定指令)、段二三個 MCP(今天主線、親手裝)、段三用得安全 + blog 寫作與部署。段二是主角,三個 MCP 一個一個裝、一個一個驗收。',
  ...
  '痛一次,第一台。AI 回你「改好了」,但你怎麼知道畫面、資料都對?你得自己開瀏覽器、自己按 F12 查 console 又查 Network,改一次查一次很累。接上 chrome-devtools MCP,讓 AI 自己開 Chrome、一次回報多項檢查。',
  'Chrome DevTools MCP 七要素卡:用途不只驗收畫面,還查 console error、動態背景、canvas 動畫、兩個推播範本;固定 prompt 一次列 4 項檢查,截圖只是附帶證據,不是重點。',
  '這張首頁截圖不只是截圖,是 AI 透過 chrome-devtools MCP 逐項檢查過的結果:console 有沒有錯、動態背景在不在、canvas 動畫跑不跑、兩個推播範本看不看得到,截圖是附帶證據。',
  ...
  'Context7 MCP 七要素卡:固定 prompt 換成「Astro 部署到 GitHub Pages 的最新設定方式?use context7」,順便替待會的 blog 部署鋪路,答案附文件出處。',
  '痛一次,第三台。專案一大,問「ShopConsole 被誰用?」AI 得一檔一檔翻、常猜錯;改東西前也不知道會不會誤傷別的檔。先 index_repository 把專案索引成地圖,架構問題秒答,也能先問影響範圍。',
  'Codebase Memory MCP 七要素卡:不只查架構總覽、呼叫關係,固定 prompt 多問一句「如果 C2 改 shopLogic.js,會影響哪些畫面」,展示不只能問架構,還能做影響範圍分析。',
  ...
  '寫完不等於做完,還要部署。push 上 GitHub 後,Settings → Pages 選 GitHub Actions,到 Actions 分頁跑一次部署 workflow,打開自己的 GitHub Pages 網址驗收。base path 由 astro.config.mjs 自動讀 repo 名稱算出,不用手改。',
  '這張是這個課程 repo 真的部署過一次的結果:build、deploy 都成功,網址算得出來、打得開。',
  ...
  '這一堂學了這些:Skills(把規矩寫成固定指令)、三個 MCP(chrome-devtools 看畫面、context7 查文件、codebase-memory 記專案)、一篇技術 blog 與它的 GitHub Pages 部署。',
...
export default [
  ...
  // blog:寫作 + 部署
  AstroBlog, BlogDeploy, BlogDeployArtifact,
  // 結業
