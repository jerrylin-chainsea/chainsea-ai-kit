...
const Foot = ({ label = 'U11 · C2 · 資料合約與 AI 工作流' }: { label?: string }) => {
...
const Timeline = () => (
  <div style={{ display: 'grid', gridTemplateColumns: '170px 1fr 1fr 280px', maxWidth: 1540, border: `1px solid ${C.line}`, borderRadius: 16, overflow: 'hidden', background: C.card }}>
    ...
    {[
      ['0:00-0:20', '回顧 C1 成品與今天主線', '啟動 web-lab', 'Dashboard 跑起來'],
      ['0:20-0:55', 'report.json 與資料合約', '讀欄位、看雙下游', '欄位規格表'],
      ['0:55-1:40', 'Plan mode / planner', '複製 Planner Prompt', 'A-F 計畫'],
      ['1:40-2:30', '人審與 implementer', '放行最小修改', '低庫存推播準備區塊'],
  ...
  <div style={{ maxWidth: 1500 }}>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 140px 1fr', gap: 18, alignItems: 'center' }}>
      <div style={{ background: '#101418', color: '#dce7df', borderRadius: 18, padding: '26px 30px', fontFamily: mono, fontSize: 23, lineHeight: 1.6 }}>
        data-lab/report.json<br />
        risk_level: high<br />
        total_revenue: 128400<br />
        anomaly_count: 5<br />
        top_channel: LINE<br />
        action_items: [...]
      </div>
      ...
      <div style={{ display: 'grid', gap: 14 }}>
        <div style={{ background: '#eef7f2', border: '1px solid #bfe6d4', borderRadius: 16, padding: '22px 26px' }}>
          <div style={{ color: C.green, fontSize: 28, fontWeight: 850 }}>web-lab Dashboard</div>
          <div style={{ fontSize: 24, color: C.muted, marginTop: 8 }}>C2 先讓畫面讀同一份資料並新增推播準備區。</div>
        </div>
        <div style={{ background: '#eef4ff', border: '1px solid #cfe0fb', borderRadius: 16, padding: '22px 26px' }}>
          <div style={{ color: C.blue, fontSize: 28, fontWeight: 850 }}>line-lab/sendLineAlert.js</div>
          <div style={{ fontSize: 24, color: C.muted, marginTop: 8 }}>C3 才進 LINE Flex payload 與 mock send。</div>
        </div>
      </div>
    </div>
  </div>
...
const DashboardMock = () => (
  <div style={{ maxWidth: 1540, background: C.card, border: `1px solid ${C.line}`, borderRadius: 20, padding: 24, boxShadow: '0 24px 70px -42px rgba(30,30,40,.48)' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${C.line}`, paddingBottom: 16 }}>
      <div>
        <div style={{ fontSize: 35, fontWeight: 900 }}>營運異常 Dashboard</div>
        <div style={{ fontSize: 22, color: C.muted, marginTop: 4 }}>report.json → 畫面 → C3 LINE Flex</div>
      </div>
      <div style={{ background: '#fff1f0', color: C.red, border: '1px solid #ffc9c4', borderRadius: 999, padding: '8px 16px', fontFamily: mono, fontWeight: 850 }}>risk: high</div>
    </div>
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 20 }}>
      {[
        ...
        <div key={k} style={{ background: '#f7f9fb', border: `1px solid ${C.line}`, borderRadius: 16, padding: '20px 22px' }}>
          <div style={{ color: C.muted, fontSize: 21 }}>{k}</div>
          <div style={{ color: C.ink, fontSize: 34, fontWeight: 900, marginTop: 6 }}>{v}</div>
        </div>
      ))}
    </div>
    <div style={{ marginTop: 18, background: '#fbf3ee', border: '1px solid #f3d6c4', borderRadius: 16, padding: '22px 26px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 18, alignItems: 'flex-start' }}>
        <div>
          <div style={{ color: C.orange, fontSize: 28, fontWeight: 900 }}>低庫存推播準備</div>
          <div style={{ color: C.muted, fontSize: 24, marginTop: 8 }}>先整理收件人會看到的摘要、商品、建議處置；不在 C2 真的送 LINE。</div>
        </div>
        <div style={{ background: C.ink, color: '#fff', borderRadius: 12, padding: '12px 16px', fontFamily: mono, fontSize: 20 }}>ready for C3</div>
      </div>
    </div>
  </div>
);
...
const Cover: Page = () => (
  <div style={{ ...fill, background: C.dark, color: '#fff' }}>
    <div style={pad}>
      ...
      <div style={{ marginTop: 28 }}><Title size={92}>用資料合約管住 AI<br />讓 Dashboard 準備好 LINE 通知</Title></div>
      <div style={{ marginTop: 28, fontSize: 34, color: 'rgba(255,255,255,.72)', lineHeight: 1.5, maxWidth: 1390 }}>今天不是自由做新網站。今天把 C1 的專案推進成可控交付：看懂 report.json，要求 AI 先出計畫，人審後才實作，最後用 build 與 diff 驗收。</div>
    </div>
    <div style={{ position: 'absolute', left: 108, bottom: 42, fontFamily: mono, color: 'rgba(255,255,255,.48)', fontSize: 20 }}>U11-C2 · Dashboard Plan</div>
  </div>
);
...
const TodayGoal: Page = () => (
  <Shell eyebrow="今日成果" title="C2 做完要留下三樣東西" lead="這三樣會直接餵給 C3，所以今天的修改要小、可驗、可交接。">
    <Cards columns={3} items={[
      { title: '一份資料合約語言', body: '知道 report.json 欄位代表什麼，risk_level 為什麼只能是 low / medium / high。' },
      { title: '一套 AI 工作流', body: 'planner 先出 A-F 計畫，人類審核後 implementer 才能動手。' },
      { title: '一個 Dashboard 準備區', body: '新增低庫存推播準備區塊，先預備 LINE Flex，C2 不真送。' },
    ]} />
  </Shell>
);
...
const Sec1: Page = () => <SectionSlide no="1" time="0:20 - 0:55" title="資料合約：一份 report.json，兩個下游" lead="先讀懂資料，才有資格要求 AI 改畫面。C2 的核心是讓資料欄位成為規格，而不是口頭感覺。" />;

const DataContract: Page = () => (
  <Shell eyebrow="段 1 · report.json" title="今天所有畫面都從這份資料來" lead="欄位名稱就是合約。AI 可以幫你排版，但不能自己發明資料來源。">
    <Code label="ai-project-foundation-kit/data-lab/report.json">{`{
...
const Fields: Page = () => (
  <Shell eyebrow="段 1 · 合約欄位" title="每個欄位都會進畫面或通知" lead="今天先讓 Dashboard 正確顯示與整理；下一堂才把同一份資料變成 LINE Flex payload。">
    <Cards items={[
      { title: 'risk_level', body: '只能是 low / medium / high。這是擋錯的第一道防線。', color: C.red },
      { title: 'total_revenue / anomaly_count', body: '營收與異常數，讓人一眼判斷嚴重程度。', color: C.blue },
      { title: 'top_product / top_channel', body: '說明異常和哪個商品、通路最相關。', color: C.orange },
      { title: 'action_items', body: '通知收件人下一步要做什麼，不能只寫漂亮摘要。', color: C.green },
    ]} />
  </Shell>
);

const TwoDownstreams: Page = () => (
  <Shell eyebrow="段 1 · 一份資料，兩道防線" title="不要複製資料，也不要讓每段程式各說各話" lead="C2 先處理 Dashboard，下堂課 C3 再處理 LINE。兩邊都必須尊重同一份合約。">
    <ContractDiagram />
  </Shell>
);

const Guardrails: Page = () => (
  <Shell eyebrow="段 1 · 允許範圍" title="今天只允許改 Dashboard 相關檔案" lead="限制範圍不是綁手綁腳，是讓 AI 的輸出可以被驗收。">
    <Cards columns={3} items={[
      { title: '可以改', body: 'web-lab/src/Dashboard.jsx\nweb-lab/src/styles.css', color: C.green },
      { title: '不該改', body: 'package.json、line-lab、data-lab 合約邏輯、任何 token 或 .env。', color: C.red },
      { title: '要保留', body: '既有 Dashboard 能跑、既有 report.json 能讀、C3 LINE 主線不被破壞。', color: C.blue },
    ]} />
  </Shell>
);
...
目標：
在營運異常 Dashboard 加上一個「低庫存推播準備」區塊。
這個區塊要為下一堂 LINE OA Flex Message 做準備，但今天不真的送出。
...
const ExpectedPlan: Page = () => (
  <Shell eyebrow="段 2 · 計畫長相" title="好的 planner 回覆應該像規格，不像作文" lead="學生審核時只看幾個關鍵點：範圍、資料、風險、驗收。">
    <Cards items={[
      { title: 'A 檔案範圍', body: '只點名 Dashboard.jsx 與 styles.css；沒有碰 line-lab 或 package.json。' },
      { title: 'B 修改內容', body: '新增低庫存推播準備區，不重寫整個 Dashboard。' },
      { title: 'C 資料合約', body: '使用 action_items、top_product、top_channel、risk_level 等欄位。' },
      { title: 'D 驗收方式', body: '畫面看得到準備區，npm run build 通過，git diff 範圍乾淨。' },
...
const Sec3: Page = () => <SectionSlide no="3" time="1:40 - 2:30" title="Implementer：小範圍修改 Dashboard" lead="人審通過後，才讓 AI 實作。這次不是重做整個頁面，而是加一個能承接 C3 的區塊。" />;

const TaskSpec: Page = () => (
  <Shell eyebrow="段 3 · 實作目標" title="新增「低庫存推播準備」區塊" lead="這個區塊要讓學生理解：C2 先準備資料與畫面，C3 才進 LINE Flex payload 與 mock send。">
    <DashboardMock />
  </Shell>
);
...
5. 不要改 data-lab/report.json 的欄位結構。

完成後請回報：
A. 修改了哪些檔案
B. 每個檔案改了什麼
C. 如何使用 report.json 欄位
D. 我應該怎麼驗收
...
const VisualRules: Page = () => (
  <Shell eyebrow="段 3 · UI 驗收" title="畫面要像工具，不像宣傳頁" lead="這是營運 Dashboard，不是 landing page。資訊要能掃描、比較、重複使用。">
    <Cards columns={3} items={[
      { title: '清楚標題', body: '低庫存推播準備，讓人知道這區塊的工作目的。' },
      { title: '資料帶入', body: '顯示商品、通路、風險、action_items，不寫死任意假資料。' },
      { title: '狀態分明', body: '標清楚「準備中 / 待人審 / C3 才送出」。' },
      { title: '不誤導', body: '不要出現「已送出 LINE」這種 C2 不會發生的狀態。' },
      { title: '不擠版', body: '桌機與窄螢幕都不能文字溢出或互相重疊。' },
      { title: '不重構', body: '既有 Dashboard 邏輯能跑，不為了漂亮而重寫整頁。' },
    ]} />
  </Shell>
);
...
cd ..
git status
git diff -- web-lab/src/Dashboard.jsx web-lab/src/styles.css

# 檢查：是否只有允許檔案被改？
# 檢查：畫面是否仍讀 report.json？
...
const Pitfalls: Page = () => (
  <Shell eyebrow="段 4 · 常見偏航" title="看到這些狀況，先停下來，不要繼續加需求" lead="C2 的錯通常不是技術不會，而是 AI 擅自把範圍擴大。">
    <Cards items={[
      ...
      { title: 'AI 寫死假資料', body: '要求改回讀 report.json 欄位，不要在 UI 裡硬塞數字。', color: C.amber },
      { title: '畫面太像行銷頁', body: '收斂成 Dashboard 區塊：標題、摘要、action items、狀態。', color: C.blue },
    ]} />
  </Shell>
);
...
- 是否改到 data-lab/report.json 欄位結構？
- 是否重寫了不必要的 Dashboard 區塊？

請只回覆：
1. 哪些修改應該保留
2. 哪些修改應該移除
3. 如何回到只修改 Dashboard.jsx / styles.css 的最小版本
4. 回復後要跑哪些驗收`}</Prompt>
  </Shell>
);
...
const FinalReview: Page = () => (
  <Shell eyebrow="段 4 · Final Reviewer" title="交付前四問" lead="這四問是 C2 的收尾，也會一路用到 C3、C4。">
    <Checklist items={[
      '畫面是否真的新增低庫存推播準備區，且內容來自 report.json？',
...
git add web-lab/src/Dashboard.jsx web-lab/src/styles.css
git commit -m "新增低庫存推播準備區塊"`}</Code>
  </Shell>
);
...
const Acceptance: Page = () => (
  <Shell eyebrow="完成定義" title="C2 看到這些才算完成" lead="如果缺任何一項，就還不是可交付狀態。">
    <Cards items={[
      { title: '資料合約', body: '能說出 report.json 欄位如何餵 Dashboard 與 C3 LINE。', color: C.green },
      { title: 'Planner', body: '有一份 A-F 計畫，且人審後才進 implementer。', color: C.blue },
      { title: 'Dashboard', body: '有低庫存推播準備區塊，沒有誤稱已送出 LINE。', color: C.orange },
...
const BridgeToC3: Page = () => (
  <Shell eyebrow="銜接下一堂" title="C3 要接手的是這個準備區，而不是重做一遍" lead="C2 的輸出會在 C3 變成 Flex payload 預覽、人工審核 checkbox、mock send 與 blocked send。">
...
export const meta: SlideMeta = {
  title: 'U11-C2: 資料合約、Planner/Implementer 與 Dashboard 推播準備',
...
export const notes: string[] = [
  '開場先把舊 C2 的自由網站製作拿掉，明確說今天改成 Dashboard 主線。',
  '說明四堂課的連續性：C2 是控制 AI 修改範圍的核心。',
  '今天三個產出：資料合約語言、AI 工作流、Dashboard 推播準備區。',
  '時間表先講清楚，前半段會刻意慢，因為要先把規格講穩。',
  '段一進資料合約。提醒學生：不知道欄位意思，就不要讓 AI 改畫面。',
  '逐欄念 report.json，特別提醒 risk_level 的允許值。',
  '把欄位和畫面、通知的用途連起來。',
  '說明一份資料同時餵 Dashboard 和 C3 的 LINE 腳本，不能複製出第二份真相。',
  '鎖定允許修改檔案：Dashboard.jsx 與 styles.css。',
  '啟動本機，先看到現況。',
  ...
  'UI 驗收聚焦 Dashboard 工具感，不要變 landing page。',
  '改完立刻 build 與 diff。',
  '段四進 reviewer、救援與收尾。',
  '列出 AI 常見偏航。',
